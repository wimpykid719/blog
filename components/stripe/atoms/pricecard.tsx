    ...
    <>
      ...
      >
        <span className="block absolute z-1 w-full p-3">
          <span className={`block mb-2 w-5 h-5 ${id === donateId ? "bg-white" : gradientColor} rounded-full inner-white`}></span>
        ...
        <span className={`block rounded-3xl position-gradient ${gradientColor} ${id === donateId ? "opacity-1" : "opacity-0"}`} />
      </label>
    </>
  )
}